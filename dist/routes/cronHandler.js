...
app.get("/send-emails", async (c) => {
    ...
    const { data: users, error } = await supabase
        ...
        .select("user_id, alert_frecuency, auto_email, allow_deepanalisis, last_notified")
        .eq("auto_email", true)
...
export async function generateEmailBody(challenges, deep) {
    const listItems = challenges
        .map((c) => `<li style="padding: 6px 0; border-bottom: 1px solid #eee;">
          <strong>${c.name}</strong>${c.completed_at
            ...
            const response = await fetch("https://api.deepseek.com/chat/completions", {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        {
                            role: "system",
                            content: "Eres un coach experto que brinda análisis motivacionales sobre hábitos y productividad.",
                        },
                        {
                            role: "user",
                            content: prompt,
