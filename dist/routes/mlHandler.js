...
app.post("/calibrate", async (c) => {
    try {
        ...
        form.append("image", new Blob([await image.arrayBuffer()], { type: image.type }), image.name);
    ...
    try {
        ...
        if (parsedData.fingers) {
            const { data: finger, error: fingerError } = await supabase
                ...
                .select("name");
            if (fingerError) {
                return c.json({ error: "Failed to update challenges for finger count" }, 500);
            }
            return c.json({
                finger,
                updated: true,
